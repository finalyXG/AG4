...
class cyclegan(object):
	...
	def _build_model(self):
		...
		self.g4,self.mask,self.gb,self.fake_A_static = self.generatorA(self, self.real_data_image,self.z, None, self.options, False,name="generatorB2A")
		...
		for i in range(self.options.frames_nb):
			self.combined_v.append( transformer(self.fake_A_static[:,i],self.mtx[i],out_size) )
		...
		self.combined_v_tf = tf.reshape(self.combined_v_tf,(-1,self.options.frames_nb)+out_size+(3,))
		...
		#self.combined_v_tf
		# ------------------------------------------------------------
		mtx0 = tf.tile(tf.constant(np.array([[0,0.76,0,0.76,0,0]])), [self.options.batch_size,1] )
		self.real_video_camera = []
		for i in range(self.frames_nb):
			self.real_video_camera.append( transformer(self.real_data_video[:,i], mtx0, out_size) )
		...
		self.disc_real, self.DA_real = self.discriminatorA(self.real_video_tf, self.real_data_image, self.options, reuse=True, name="discriminatorA")
		...
		self.d_loss = 0 * tf.reduce_mean(self.DA_wi_rv) + tf.reduce_mean(fake_logit) - tf.reduce_mean(true_logit)
		...
		for epoch_batch in range(0,200): #args.epoch
			...
			for vid in idx:
				...
				for i in range(2,self.bd.shape[0] - 1,1):
					...
					current_shot_f1 = self.bd[i] + 2 #+ np.random.randint(2,size=1)[0]
					...
					l = range(self.bd[i]+1,self.bd[i]+6)
					...
					if over == 0:
						...
						if  abs(np.mean(tmp)) < 0.001 or abs(np.mean(a_video_one[1])) < 0.005 :
							a_video_one == []
							continue
					...
					if flag_camera == 0:#self.fake_A fake_A
						fake_A,g4,gmask,gb, = self.sess.run([self.fake_A,self.g4,self.mask,self.gb],feed_dict={self.real_data_image: merge_image,self.z:batch_z})
						g4 = np.array(g4)
						...
						####################
							print("====Update Critic====") 
							for j in range(Diter):
								_, summary_str,_ = self.sess.run([self.d_optim, self.d_sum,self.d_clamp_op],\
													 feed_dict={self.z: batch_z,\
													 self.fake_data_image: merge_image_false,\
													 self.real_data_image: merge_image,self.real_data_video:a_video}) #
								self.writer.add_summary(summary_str, counter) 
					...
					if tmp_rand <= 0.2:
						...
						#pdb.set_trace()
						save_images([[ np.squeeze(merge_image[0,:,:,0:3]) ]],[self.batch_size, 1],'./{}/{}/A_{}_{:02d}_{:03d}.jpg'.format(args.sample_dir,epoch,i,1,epoch))
						#save_images([[ np.squeeze(merge_image[0,:,:,3:6]) ]],[self.batch_size, 1],'./{}/{}/A_{}_{:02d}_{:03d}.jpg'.format(args.sample_dir,epoch,i,2,epoch))
						...
						make_gif(np.squeeze(np.array(a_video[0])),'./{}/{}/A_{}_{:02d}_{:03d}.gif'.format(args.sample_dir,epoch,i,4,epoch))
						make_gif(np.array(pred_video[0]),'./{}/{}/A_{}_{:02d}_{:03d}.gif'.format(args.sample_dir,epoch,i,5,epoch))
